...
# Example training script for Sokoban Coding Agent using Torch multi-turn rollout
# Runs grl/verl_grpo_train.py with overrides, modeled after verl_quick_training_example.sh
...
EXPERIMENT_NAME=${10:-"coding_agent_train_$(date +"%Y%m%d_%H%M%S")"}
MODEL_PATH=${11:-"Qwen/Qwen2.5-0.5B-Instruct"}
TOTAL_TRAINING_STEPS=${12:-50}

echo "=== Coding Agent Training ==="
...
PROJECT_ROOT="$(pwd)"
mkdir -p cache
LOG_FILE="cache/coding_agent_train_$(date +"%Y%m%d_%H%M%S").log"
...
python grl/verl_grpo_train.py \
  --config-name "grpo_base" \
