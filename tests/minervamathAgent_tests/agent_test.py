...
from grl.agents.minervamathAgent.agent import MinervamathAgent
import yaml
...
def setup_logging():
  ...
  tee = Tee(log_path)
  sys.stdout = tee
  print(f"📝 MinervamathAgent Test log started at {datetime.now()}")
...
def test_agent_creation():
  print("🔍 Testing MinervamathAgent creation …")
  cfg = load_config()
...
def test_agent_reset_and_step():
  ...
  conf["agent_type"] = "minervamathAgent"
  conf["env_config"] = {"dataset_path": "math-ai/minervamath", "split": "test"}
  ag = MinervamathAgent(conf, agent_id=0, group_id=0)
...
def test_single_rollout():
  ...
  conf["agent_type"] = "minervamathAgent"
  conf["env_config"] = {"dataset_path": "math-ai/minervamath", "split": "test"}
  ag = MinervamathAgent(conf, agent_id=0, group_id=0, seed=0)
  env_out = ag.reset()
