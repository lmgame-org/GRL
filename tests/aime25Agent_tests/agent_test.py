...
from grl.agents.aime25Agent.agent import AIME25Agent
import yaml
...
def setup_logging():
  ...
  log_path = log_dir / f"aime25_agent_test_{ts}.log"
  ...
  tee = Tee(log_path)
  sys.stdout = tee
  print(f"📝 AIME25Agent Test log started at {datetime.now()}")
...
def test_agent_creation():
  print("🔍 Testing AIME25Agent creation …")
  cfg = load_config()
  # reuse single-turn config structure
  conf = cfg["math500Agent_single_turn"].copy()
  conf["agent_type"] = "aime25Agent"
  conf["env_config"] = {"dataset_path": "math-ai/aime25", "split": "test"}
  ag = AIME25Agent(conf, group_id=0, agent_id=0, seed=42, tag="TestAIME25")
...
def test_agent_reset_and_step():
  ...
  conf["agent_type"] = "aime25Agent"
  conf["env_config"] = {"dataset_path": "math-ai/aime25", "split": "test"}
  ag = AIME25Agent(conf, agent_id=0, group_id=0)
...
def test_single_rollout():
  ...
  conf["agent_type"] = "aime25Agent"
  conf["env_config"] = {"dataset_path": "math-ai/aime25", "split": "test"}
  ag = AIME25Agent(conf, agent_id=0, group_id=0, seed=0)
  env_out = ag.reset()
