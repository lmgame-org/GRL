...
def setup_logging():
  ...
  log_path = log_dir / f"aime25_env_test_{ts}.log"
  ...
  tee = Tee(log_path)
  sys.stdout = tee
  print(f"📝 AIME25Env Test log started at {datetime.now()}")
  print(f"📄 Log file: {log_path}")
  print("=" * 60)
  return tee


class AIME25EnvConfig:
  def __init__(self):
    self.dataset_path = "math-ai/aime25"
    self.split = "test"
    self.max_steps = 5
...
def get_default_config():
  cfg = AIME25EnvConfig()
  print("✅ Using default AIME25 configuration")
...
def test_optional_equivalences():
  print("🔍 Test 3: optional equivalence cases (tuple spacing)")
  env = make_env(get_default_config())
  env.reset(seed=321)
  gold = str(env.correct_answer)
