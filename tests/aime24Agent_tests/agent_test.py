...
from grl.agents.aime24Agent.agent import AIME24Agent
import yaml
...
def setup_logging():
  ...
  log_path = log_dir / f"aime24_agent_test_{ts}.log"
  ...
  tee = Tee(log_path)
  sys.stdout = tee
  print(f"📝 AIME24Agent Test log started at {datetime.now()}")
...
def test_agent_creation():
  print("🔍 Testing AIME24Agent creation …")
  cfg = load_config()
  ag = AIME24Agent(cfg["math500Agent_single_turn"], group_id=0, agent_id=0, seed=42, tag="TestAIME24")
...
def test_agent_reset_and_step():
  ...
  cfg = load_config()
  # reuse math500 single-turn config structure; only env dataset differs
  conf = cfg["math500Agent_single_turn"].copy()
  conf["agent_type"] = "aime24Agent"
  conf["env_config"] = {"dataset_path": "math-ai/aime24", "split": "test"}
  ag = AIME24Agent(conf, agent_id=0, group_id=0)
...
def test_single_rollout():
  ...
  conf["agent_type"] = "aime24Agent"
  conf["env_config"] = {"dataset_path": "math-ai/aime24", "split": "test"}
  ag = AIME24Agent(conf, agent_id=0, group_id=0, seed=0)
  env_out = ag.reset()
