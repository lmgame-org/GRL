...
from grl.agents.math500Agent.env import Math500Env
...
def setup_logging():
  ...
  tee = Tee(log_path)
  sys.stdout = tee
  print(f"📝 Math500Env Test log started at {datetime.now()}")
...
def get_default_config():
  cfg = Math500EnvConfig()
  print("✅ Using default Math500 configuration")
...
def make_env(cfg_obj):
  return Math500Env(vars(cfg_obj))
...
def test_step_logic_optional_equivalences():
  ...
  # base notation: like 52_8 → 42
  m = re.fullmatch(r"([0-9A-Za-z]+)_([0-9]{1,2})", gold.replace(" ", ""))
  if m:
    try:
      dec = int(m.group(1), int(m.group(2)))
      _, r, done, info = env.step(str(dec))
      assert info["success"] is True and done is True and r > 0
      print(f"   Base accepted: {gold} ≈ {dec}")
    except Exception:
      print("   Base case present but conversion failed (skipped)")
