...
def setup_logging():
  ...
  log_path = log_dir / f"amc23_env_test_{ts}.log"
  ...
  tee = Tee(log_path)
  sys.stdout = tee
  print(f"📝 AMC23Env Test log started at {datetime.now()}")
  print(f"📄 Log file: {log_path}")
  print("=" * 60)
  return tee


class AMC23EnvConfig:

  def __init__(self):
    self.dataset_path = "math-ai/amc23"
    self.split = "test"
    self.max_steps = 5
...
def get_default_config():
  cfg = AMC23EnvConfig()
  print("✅ Using default AMC23 configuration")
...
def test_step_logic_wrappers():
  ...
  candidates = [gold, f"$ {gold} $", f"\\boxed{{{gold}}}", f"\\text{{{gold}}}"]
...
if __name__ == "__main__":
  ...
  try:
    ...
    print()
    print("=" * 60)
    print("🎉 All AMC23Env tests passed!")
