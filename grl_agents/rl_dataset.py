...
class RLDataset:
  ...
  def __init__(self, base_config: Dict[str, Any], groups_per_batch: int = 1, seeds_per_group: int = 1, seed_start: int = 0):
  ...
  def get_batch(self, index: int) -> List[AgentGroupBuilder]:
    ...
    for g in range(self.groups_per_batch):
      seeds = [self.seed_start + index * self.seeds_per_group + i for i in range(self.seeds_per_group)]
