...
class BaseAgent:
  ...
  def __init__(self, config: Dict[str, Any], group_id=0, agent_id=0, seed=None, tag=None):
  ...
  def parse_llm_response(self, llm_response: str, enable_think: bool = True) -> Tuple[str, List[str]]:
    import re
    ...
    actions = [a.strip() for a in action_content.split(self.action_separator) if a.strip()]
    ...
    processed = f"<think>{think_content}</think><answer>{action_content}</answer>" if enable_think else f"<answer>{action_content}</answer>"
  ...
  def get_final_rollout_states(self) -> Dict[str, Any]:
    ...
    action_is_effective_values = [traj.info.get("action_is_effective", False) for traj in dq]
    metrics[f"{self.tag or 'baseAgent'}/action_is_effective"] = (sum(action_is_effective_values) / len(action_is_effective_values)) if action_is_effective_values else 0.0
    action_is_valid_values = [traj.info.get("action_is_valid", False) for traj in dq]
    metrics[f"{self.tag or 'baseAgent'}/action_is_valid"] = (sum(action_is_valid_values) / len(action_is_valid_values)) if action_is_valid_values else 1.0
  ...
  def reset(self, seed: int | None = None) -> EnvOutput:
    ...
    return EnvOutput(truncated=False, terminated=False, state=obs, reward=0.0, info={})
