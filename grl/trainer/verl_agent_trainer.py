...
from rollout.torch_sync_rollout import TorchSyncRollout
...
class AgentTrainer(RayPPOTrainer):
  ...
  def init_multi_turn_rollout(self):
    ...
    # Initialize multi-turn rollout manager - Torch backend
    self.multi_turn_rollout = TorchSyncRollout(
    ...
    self.validation_multi_turn_rollout = TorchSyncRollout(
        actor_rollout_wg=self.actor_rollout_wg,
        cfg=self.config,
        tokenizer=self.tokenizer,
        validation=True,
  ...
  def _generate_multi_turn_sequences(
    ...
    if (
        self.multi_turn_rollout is None
        or self.validation_multi_turn_rollout is None
    ):
      self.init_multi_turn_rollout()

    # Type narrowing assertion - we know it's not None after initialization
    assert (
        self.multi_turn_rollout is not None
    ), "multi_turn_rollout should be initialized"

    # Run multi-turn rollout to get complete trajectories
    self.multi_turn_rollout.rollout()

    # Build PPO batch directly from rollout states (no filtering in verl trainer)
    final_rollout_states = (
        self.multi_turn_rollout._collect_final_rollout_states()
    )
    ...
    # No filtering metrics; return empty dict for compatibility
      ...
      for batch_dict in self.train_dataloader:
        ...
        with marked_timer("step", timing_raw):
          ...
          with marked_timer("gen", timing_raw, color="red"):
            ...
            batch, rollout_metrics = self._generate_multi_turn_sequences(
                gen_batch
            )
  ...
  def _validate(self):
    ...
    for step in range(self.config.trainer.validation_steps):
      total_validation_agents = (
          self.validation_multi_turn_rollout.total_agent_num
      )
      ...
      # Type narrowing assertion - we know it's not None after initialization
      assert (
          self.validation_multi_turn_rollout is not None
      ), "multi_turn_rollout should be initialized"

      # MODIFICATION: Call rollout() first, then build_ppo_batch()
      self.validation_multi_turn_rollout.rollout()
      rollout_states = (
          self.validation_multi_turn_rollout._collect_final_rollout_states()
      )
      test_batch = self.validation_multi_turn_rollout.build_ppo_batch(
          rollout_states
      )
  ...
  # ─────────────────── MODIFICATION: Save checkpoint, remove dataloader ───────────────────
  def _save_checkpoint(self):
    """
    We have no dataloader so we won't save it. Other logic is the same.
    """
