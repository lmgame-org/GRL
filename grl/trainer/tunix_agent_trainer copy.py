...
class PpoLearnerExp(PpoLearner):
  ...
  # ─────────────────── MODIFICATION: add a helper function to convert metrics_logger.Mode to rl_cluster_lib.Mode ───────────────────
  def _to_rl_mode(self, mode: metrics_logger.Mode) -> rl_cluster_lib.Mode:
    return (
  ...
  def _generate_and_compute_advantage(
    ...
    # ===== Metric logging (buffer at global steps) ======
    rl_mode = self._to_rl_mode(mode)
      ...
      try:
        for name, value in mt_metrics_dict.items():
          self.rl_cluster.buffer_metrics(
              {name: (float(value), np.mean)}, mode=rl_mode
          )
      except Exception:
        pass
    ...
    self.rl_cluster.buffer_metrics(
        {
            "raw_reward/mean": (np.mean(last_token_scores), np.mean),
            "raw_reward/max": (np.max(last_token_scores), np.max),
            "raw_reward/min": (np.min(last_token_scores), np.min),
        },
        mode=rl_mode,
    )
    ...
    self.rl_cluster.buffer_metrics(
        {
            "final_reward/mean": (np.mean(sequence_rewards), np.mean),
            "final_reward/max": (np.max(sequence_rewards), np.max),
            "final_reward/min": (np.min(sequence_rewards), np.min),
        },
        mode=rl_mode,
    )
    if self.ppo_config.beta != 0.0:
      ...
      self.rl_cluster.buffer_metrics(
          {
              "actor/reward_kl_penalty": (
                  float(per_sequence_mean_kl.mean()),
                  np.mean,
              )
          },
          mode=rl_mode,
    ...
    try:
      ...
      self.rl_cluster.buffer_metrics(
          {
              "old_values/mean": (float(_mean_vals), np.mean),
              "old_values/min": (float(_min_vals), np.min),
              "old_values/max": (float(_max_vals), np.max),
          },
          mode=rl_mode,
      )
    except Exception:
    ...
    try:
      ...
      self.rl_cluster.buffer_metrics(
          {
              "advantages/mean": (float(_adv_mean), np.mean),
              "advantages/min": (float(_adv_min), np.min),
              "advantages/max": (float(_adv_max), np.max),
              "returns/mean": (float(_ret_mean), np.mean),
              "returns/min": (float(_ret_min), np.min),
              "returns/max": (float(_ret_max), np.max),
          },
          mode=rl_mode,
      )
    except Exception:
    ...
    self.rl_cluster.buffer_metrics(
        {
            "completions/mean_length": (np.mean(agg_completion_mask), np.mean),
            "completions/max_length": (np.max(agg_completion_mask), np.max),
            "completions/min_length": (np.min(agg_completion_mask), np.min),
        },
        mode=rl_mode,
    )
    ...
    while True:
      try:
        while (
            ...
            and self._iter_steps < self._last_train_step
        ):
          next(iterator)
          self._iter_steps += 1
  ...
  def _validate(self, eval_ds: Iterable[_TrainingInputT] | None) -> None:
    ...
    # Log evaluation metrics at global steps
    try:
    ...
    print(
        f"[VALIDATION] Done | logged {len(metrics_dict)} metrics at step={self.rl_cluster.actor_trainer.train_steps}"
    )
  ...
  def train(
    ...
    while True:  # loop over M
      try:
        ...
        initial_train_steps = self._iter_steps
        ...
        if eval_every and eval_every > 0:
          ...
          if curr_q > prev_q and self.rl_cluster.actor_trainer.train_steps > 0:
            print(
                f"[VALIDATION] Triggering (crossed interval) at train_step={self.rl_cluster.actor_trainer.train_steps} (eval_every_n_steps={eval_every})"
            )
            self._validate(None)
...
def ppo_policy_loss_fn(
  ...
  per_token_logps, logits_slice = common.compute_per_token_logps(
      model,
      prompt_tokens=prompt_ids,
      completion_tokens=completion_ids,
      pad_id=pad_id,
