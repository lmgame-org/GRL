...
class BirdEnv(BaseEnv):
  ...
  def __init__(self, config):
    ...
    self._step_timeout_s: float = float(
        self.config.get("step_timeout_seconds", 180)
    )
  ...
  def _db_file(self) -> str:
    return os.path.join(
        self.config["db_root"], self.db_id, f"{self.db_id}.sqlite"
    )
  ...
  def _wait_for_db_ready(self) -> None:
    ...
    if (
        self._db_ready
        and self._last_db_id == self.db_id
  ...
  def _execute_sql(
  ...
  ) -> Tuple[bool, Union[List[Tuple[Any, ...]], str]]:
    ...
    try:
      ...
      eff_timeout = float(
          self._step_timeout_s if timeout_s is None else timeout_s
      )
      ...
      rows_sorted = sorted(
          rows,
          key=lambda row: [x if x is not None else float("-inf") for x in row],
      )
  ...
  def step(self, action: str):
    ...
    if not match:
      ...
      info = {"action_is_valid(code_block)": False, "success": False}
    ...
    sub_ok, sub_res_or_err = self._execute_sql(
        submitted_sql, timeout_s=self._step_timeout_s
    )
    if not sub_ok:
      # Timed out or errored; format observation and continue multi-turn
      timed_out = isinstance(sub_res_or_err, str) and sub_res_or_err.startswith(
          "TIMEOUT("
      )
      if timed_out:
        observation = (
    ...
    gold_ok, gold_res_or_err = self._execute_sql(
        self.gold_sql, timeout_s=self._step_timeout_s
    )
    ...
    info = {"action_is_valid(code_block)": True, "success": result_match}
