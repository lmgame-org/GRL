...
class TetrisEnv(BaseEnv):
  ...
  def __init__(self, config=None, **kwargs):
    ...
    if self.config.get("box_type", 1) == 2:
      self.shapes = {
          "I": [(0, 0), (0, -1)],
          "—": [(0, 0), (-1, 0)],
      }
      self.shape_names = ["I", "—"]
    elif self.config.get("box_type", 1) == 3:
      self.shapes = {
          "I": [(0, 0), (0, -1)],
          "—": [(0, 0), (-1, 0)],
          "O": [(0, 0), (-1, 0), (0, -1), (-1, -1)],
    ...
    elif self.config.get("box_type", 1) == 1:
      self.shapes = {
          "O": [(0, 0)],
      }
      self.shape_names = ["O"]
  ...
  def _new_piece(self):
    ...
    if self.current_piece_index < len(self.pre_generated_pieces):
      self.anchor, self.shape = self.pre_generated_pieces[
  ...
  def step(self, action: int) -> Tuple[Any, float, bool, Dict]:
    ...
    previous_pos = (
        copy.deepcopy(self.anchor) if self.anchor is not None else None
    )
  ...
  def render(self, mode: str = "text") -> Any:
    ...
    if mode == "text":
      ...
      board_str = "\n".join(
          ["".join(["#" if j else "_" for j in i]) for i in self.board.T]
      )

      # Then add the current piece as 'X'
      self._set_piece(True)
      current_piece_positions = [
          (self.anchor[0] + x, self.anchor[1] + y) for x, y in self.shape
      ]
      self._set_piece(False)
  ...
  def close(self) -> None:
    ...
    (Nothing to free right now, but keeps test suites happy.)
    """
    self.board = None
    self.anchor = None
    self.shape = None
