...
from grl.agents.aime25Agent.env import AIME25Env
from grl.agents import register_agent


@register_agent("aime25Agent")
class AIME25Agent(BaseAgent):
  ...
  def __init__(self, config, group_id=0, agent_id=0, seed=None, tag=None):
    ...
    if self.agent_config.get("use_custom_prompt", False):
      if self.enable_think:
        self.turn_prompt_template = """Incorrect Answer.\nQuestion:\n{state}\nPlease think again. Always output: <think> [Your thoughts] </think> [your answer] with no extra text. Strictly follow this format. Max response length: {max_tokens} tokens.\n"""
      else:
        self.turn_prompt_template = """Incorrect Answer.\nQuestion:\n{state}\nPlease think again. Output [your answer] with no extra text. Strictly follow this format. Max response length: {max_tokens} tokens.\n"""

  def initialize_env(self):
    self.env = AIME25Env(self.env_config)
