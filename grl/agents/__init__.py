...
def register_agent(name: str):
  ...
  def decorator(cls):
    if name in REGISTERED_AGENTS and REGISTERED_AGENTS[name] != cls:
      raise ValueError(
          f"Agent {name} has already been registered: {REGISTERED_AGENTS[name]} vs {cls}"
      )
...
_safe_import(
    lambda: __import__(
        "grl.agents.blocksworldAgent.agent", fromlist=["BlocksworldAgent"]
    ),
    "blocksworldAgent",
)
_safe_import(
    lambda: __import__(
        "grl.agents.tetrisAgent.agent", fromlist=["TetrisAgent"]
    ),
    "tetrisAgent",
)
_safe_import(
    lambda: __import__(
        "grl.agents.webshopAgent.agent", fromlist=["WebShopAgent"]
    ),
    "webshopAgent",
)
_safe_import(
