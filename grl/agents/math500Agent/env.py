...
class Math500Env(BaseEnv):
  ...
  def _maybe_base_equal(cls, gold: str, user: str) -> bool:
    ...
    try:
      base = int(base_str)
      gold_val = int(digits, base)
    except ValueError:
      return False
    if cls._canonical_atomic(user) == cls._canonical_atomic(gold):
      return True
    try:
      user_val = int(cls._canonical_atomic(user), 10)
      return user_val == gold_val
    except ValueError:
      ok, val = cls._try_eval_numeric(user)
      return ok and abs(val - gold_val) < 1e-9
  ...
  def __init__(self, config, **kwargs):
    super(Math500Env, self).__init__()
    self.config = config
    self.dataset = load_dataset(
        self.config.get("dataset_path", "HuggingFaceH4/MATH-500"),
  ...
  def reset(self, seed=None):
    ...
    # MATH-500 uses 'problem' and 'answer' columns
    self.current_question = question_data.get("problem", "")
    self.correct_solution = question_data.get("answer", "")
