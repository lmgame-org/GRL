...
class SyncMultiTurnRollout:
  ...
  # ─────────────────── INITIALIZATION ───────────────────
  def __init__(
      self, actor_rollout_wg, cfg, tokenizer, processor, validation=False
  ):
  ...
  def get_batch_llm_prompts(self, env_outputs):
    ...
    with ThreadPoolExecutor(max_workers=self.num_prompt_threads) as ex:
      ...
      if self.show_tqdm:
        iterator = tqdm(
            iterator,
            total=len(llm_input_texts),
            desc="rollout/tokenize",
            leave=False,
        )
  ...
  def get_batch_env_outputs(self, lm_outputs):
    ...
    with ThreadPoolExecutor(max_workers=self.num_env_threads) as ex:
      ...
      if self.show_tqdm:
        iterator = tqdm(
            iterator, total=len(replies), desc="rollout/env", leave=False
        )
  ...
  def get_masks_and_scores(
    ...
    response_mask = (turn_indicators % 2 == 1) & (
        turn_indicators > 1
  ...
  def _normalize_score_tensor(
    ...
    if method == "mean_std":
      norm_func = (
          lambda x: (x - x.mean(dim=-1, keepdim=True))
    ...
    penalty = torch.tensor(
        [env_output["penalty"] for env_output in env_outputs],
        dtype=torch.float32,
    )
  ...
  def filter_rollout(self, rollout_batch: DataProto) -> DataProto:
    ...
    if self.cfg.rollout.rollout_filter_type == "std_rev":
      top_groups = (
          (-in_group_std).topk(int(rollout_filter_ratio * num_groups)).indices
      )
    elif self.cfg.rollout.rollout_filter_type == "std":
      top_groups = in_group_std.topk(
          int(rollout_filter_ratio * num_groups)
      ).indices
    else:
      raise ValueError(
    ...
    for key, value in rollout_batch.non_tensor_batch.items():
      if isinstance(value, np.ndarray):
        ...
        rollout_batch.non_tensor_batch[key] = [
            v for v, m in zip(value, mask) if m
        ]
